define([
  ...
  .extend({
    ...
    destroy: function() {
      this._operators = this._lastinput = null;
      while (this.length) {
        this.pop();
      }
    },
    ...
    _run: function(operator, input) {
      var output, state, plugins = this.constructor.plugins;
      ...
      for (var i = 0; i < this.length; ++i) {
        ...
        output = this[i].main(input, plugins);
        this[i].state = state;
    ...
    clearCache: function() {
      mori.each(mori.vals(this._opcache), function(op) {
        op.destroy();
      });
      this._opcache = mori.empty(this._opcache);
    },
    plugins: {
      Math: Math
    }
